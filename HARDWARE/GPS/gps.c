...
void NMEA_GPRMC_Analysis(nmea_msg *gpsx,u8 *buf)
{
	...
	if(posx!=0XFF)
	{
		...
		//时区时间矫正
		if(gpsx->utc.hour>=24-gpsx->utc.time_zone) gpsx->utc.hour-=24-gpsx->utc.time_zone;
		else gpsx->utc.hour+=gpsx->utc.time_zone;
	}	
	...
	if(posx!=0XFF)
	{
		...
		if(gpsx->utc.time_zone>12) gpsx->utc.date-=1;
	}
}
//分析GPVTG信息
//gpsx:nmea信息结构体
